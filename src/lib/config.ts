...
const envSchema = z.object({
  ...
  TEXT_QUALITY_MIN_SCORE: z.coerce.number().min(0).max(100).default(75),
  ...
  OCR_MAX_CONCURRENCY: z.coerce.number().int().min(1).max(2).default(2),
  OCR_RENDER_DPI: z.coerce.number().int().min(72).max(400).default(300),
...
export const config = {
  ...
  RESUME_DUAL_CHANNEL_ON_WARNING: parsed.RESUME_DUAL_CHANNEL_ON_WARNING === "true",
  ALIYUN_TABLE_OCR_LINELESS: parsed.ALIYUN_TABLE_OCR_LINELESS === "true",
